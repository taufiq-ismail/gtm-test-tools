describe('Data Tracker NHP', function () {
        ...
        cy.window().then((win) => {
            ...
            if (listingHot) {
                cy.fixture('NHP/hp-hot-properties').then((hotproperties) => {
                    ...
                    assert.equal(listingHot.listingImpressions.length, hotproperties.listingImpressions.length);
                });
            }
        });
    });
    ...
    it('New Launch Home Page Data Tracker - Action', function () {
        ...
        cy.wait(3000).get('.rui-icon-arrow-right-small').eq(1).click({ force: true }).wait(1000);
        cy.window().then((win) => {
            assert.isDefined(win.dataLayer, 'Data Layer is defined');
