    ...
    it("Is send tracker listing click when user click one of selected news card", () => {
      ...
      cy.window().then((win) => {
        const selectedNews = win.dataLayer.find(
          ({ event, source }) =>
            source === "NHP-Featured Article" && event === "navigationInternal"
        );

        cy.fixture(
          "new-launch-homepage/news-section/click-news-non-login"
        ).then((fixture) => {
  ...
  describe("Testing Login", () => {
    ...
    it("Is send tracker listing click when user click one of selected news card", () => {
      ...
      cy.window().then((win) => {
        ...
        cy.fixture("new-launch-homepage/news-section/click-news-login").then(
          (fixture) => {
            assert.isDefined(selectedNews);
            assert.deepEqual(selectedNews, fixture);
          }
